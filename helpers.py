...
def writeData(f, algo, avgCPUBurstTime, avgWaitTime, avgTurnaroundTime, numContextSwitches, numPreemptions, CPUUtilization):
    ...
    f.write(f'-- average CPU burst time: {"%.3f" % round(math.ceil(1000 * avgCPUBurstTime + 0.5) / 1000, 3)} ms\n')
    f.write(f'-- average wait time: {"%.3f" % (round(1000 * avgWaitTime + 0.5) / 1000)} ms\n')
    f.write(f'-- average turnaround time: {"%.3f" % (math.ceil(1000 * avgTurnaroundTime + 0.4) / 1000)} ms\n')
    f.write(f'-- total number of context switches: {numContextSwitches}\n')
    f.write(f'-- total number of preemptions: {numPreemptions}\n')
    f.write(f'-- CPU utilization: {"%.3f" % (round(1000 * CPUUtilization + 0.5) / 1000)}%\n')